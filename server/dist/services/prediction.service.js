...
exports.predictCommissionForDeal = predictCommissionForDeal;
...
// ---------- Internal helpers ----------
function getMonthsBack(period, count) {
...
function determineConfidence(dataPoints, hasAccumulation) {
    if (dataPoints >= MAX_HISTORY_MONTHS && hasAccumulation)
        return 'high';
    if (dataPoints >= MIN_HISTORY_MONTHS)
        return 'medium';
    return 'low';
}
 ...
 */
function predictMonthlySalary(agentId, period, taxStatus, activePolicies, historicalCommissions, accumulationBalances, advanceBalance) {
    const assumptions = [];
    const relevantPeriods = getMonthsBack(period, MAX_HISTORY_MONTHS);
    const relevantHistory = historicalCommissions.filter((h) => relevantPeriods.includes(h.period));
    const grouped = groupByType(relevantHistory);
    // --- Nifraim (recurring on collected premiums) ---
    let nifraimPrediction;
    ...
    if (accumulationBalances.length > 0) {
        const growthRate = calculateGrowthRate(grouped.accumulation.length >= 2 ? grouped.accumulation : [0]);
    ...
    if (accumulationBalances.length > 0) {
        managementFeesPrediction = accumulationBalances.reduce((sum, bal) => sum + bal.closingBalance * (bal.managementFeeRate / 100), 0);
    ...
    const recentNewPolicies = activePolicies.filter((p) => recentPeriods.some((rp) => p.startDate.startsWith(rp)));
    if (recentNewPolicies.length > 0) {
        const avgOneTime = recentNewPolicies.reduce((sum, p) => sum + p.premiumAmount * (p.commissionPct / 100), 0) / MIN_HISTORY_MONTHS;
    ...
    const tax = (0, tax_service_js_1.calculateTax)(Math.max(grossPrediction, 0), taxStatus);
    const dataPointsUsed = relevantHistory.length + accumulationBalances.length;
    const confidence = determineConfidence(relevantHistory.length, accumulationBalances.length > 0);
    return {
        agentId,
        period,
        breakdown,
 ...
 */
function predictCommissionForDeal(dealType, premiumAmount, insuranceCompany, agentTaxStatus, currentGrossMonthly, overrideRates) {
    ...
    const expectedAnnualTotal = expectedOneTime +
    ...
    const currentTax = (0, tax_service_js_1.calculateTax)(currentGrossMonthly, agentTaxStatus);
    const newTax = (0, tax_service_js_1.calculateTax)(newGross, agentTaxStatus);
