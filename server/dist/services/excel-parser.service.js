...
exports.parseExcelBufferByType = parseExcelBufferByType;
const XLSX = __importStar(require("xlsx"));
const uuid_1 = require("uuid");
const phoenix_parser_service_js_1 = require("./phoenix-parser.service.js");
const analyst_parser_service_js_1 = require("./analyst-parser.service.js");
// ============ Sheet Detection ============
...
function detectReportType(sheetNames) {
...
function parseBranchDistribution(rows) {
    const records = [];
    const errors = [];
    let skipped = 0;
    for (let i = 0; i < rows.length; i++) {
...
function parseProductDistribution(rows) {
    const records = [];
    const errors = [];
    let skipped = 0;
    for (let i = 0; i < rows.length; i++) {
...
function withReportType(parser, overrideType) {
    return (rows) => {
...
function detectCompany(rows, reportType) {
...
function parseSheet(workbook, sheetName, reportType) {
...
function parseExcelBuffer(buffer) {
    ...
    if ((0, analyst_parser_service_js_1.isAnalystFormat)(workbook)) {
        const results = (0, analyst_parser_service_js_1.parseAnalystWorkbook)(workbook);
        if (results.length > 0)
            return results;
    }
    // 2. Check Phoenix format (same sheet name "דוח נפרעים" but different headers)
    if ((0, phoenix_parser_service_js_1.isPhoenixFormat)(workbook)) {
        const results = (0, phoenix_parser_service_js_1.parsePhoenixWorkbook)(workbook);
        if (results.length > 0)
            return results;
    }
    ...
    if (results.length === 0) {
        throw new Error(`No recognized sheet names found. Expected one of: ${Object.keys(SHEET_NAME_MAP).join(', ')}. ` +
 ...
 */
function parseExcelBufferByType(buffer, expectedType) {
    const workbook = XLSX.read(buffer, { type: 'buffer', codepage: 65001 });
    ...
    if (!matchedSheet) {
        throw new Error(`Sheet "${expectedSheetName}" not found. Available sheets: ${workbook.SheetNames.join(', ')}`);
    }
