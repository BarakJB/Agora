...
import { getMonthlySalarySummary } from '../repositories/sales.repository.js';
...
function deriveConfidence(months: number): ForecastConfidence {
  if (months >= 6) return 'high';
  if (months >= 3) return 'medium';
  return 'low';
}

export async function predictNextMonthFromTransactions(
  agentId: string,
  ...
  const nifraimByMonth = new Map<string, number>();
  const hekefByMonth = new Map<string, number>();
  const accumulationByMonth = new Map<string, number>();

  for (const r of rows) {
    if (!sortedMonths.includes(r.processingMonth)) continue;
    const m = r.processingMonth;
    if (r.reportType === 'nifraim') nifraimByMonth.set(m, (nifraimByMonth.get(m) ?? 0) + r.total);
    else if (r.reportType === 'hekef') hekefByMonth.set(m, (hekefByMonth.get(m) ?? 0) + r.total);
    else if (r.reportType.startsWith('accumulation')) accumulationByMonth.set(m, (accumulationByMonth.get(m) ?? 0) + r.total);
  }

  const toSortedValues = (map: Map<string, number>): number[] =>
    sortedMonths.map((m) => map.get(m) ?? 0).reverse();
