...
export interface AgreementRate {
  ...
  agreementType: string;       // e.g., "חיים ובריאות", "גמל והשתלמות"
  product: string;             // e.g., "מוצרי ריסק", "בריאות", "פנסיה מנהלים"
  commissionType: string;      // "טיפול" or "רכישה"
  rate: number;                // percentage (e.g., 0.18 = 18%)
  yearRange: string | null;    // e.g., "1-5", "6-15", "16+"
  isBookCommission: boolean;   // עמלת ספר vs תוספת
  notes: string | null;
}
 ...
 */
export async function parseAgreementPdf(buffer: Buffer): Promise<AgreementParseResult> {
...
function extractAgentInfo(text: string): { name: string | null; id: string | null; agency: string | null } {
  ...
  for (const pattern of namePatterns) {
    ...
    for (const m of matches) lastMatch = m;
    if (lastMatch) {
...
function extractDates(text: string): { from: string | null; to: string | null } {
  ...
  if (dateMatch) return { from: dateMatch[1], to: dateMatch[2] };
...
function parsePhoenixAgreement(text: string): AgreementRate[] {
...
function extractAgreementId(text: string): string | null {
 ...
 */
function parsePhoenixGamalRates(text: string, agreementId: string | null): AgreementRate[] {
  const rates: AgreementRate[] = [];
 ...
 */
function parsePhoenixLifeHealthRates(text: string, agreementId: string | null): AgreementRate[] {
  const rates: AgreementRate[] = [];
  const company = 'הפניקס';
  ...
  const tipulProducts: Array<{ name: string; pattern: RegExp }> = [
